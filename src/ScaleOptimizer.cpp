...
ScaleOptimizer::ScaleOptimizer(int w_, int h_, const Eigen::Matrix3f &K, const SE3& T_stereo_, float accept_th_)
{
    ...
    IOWrap::Output3DWrapper* wrap)
{
  ...
  for(int lvl=coarsestLvl; lvl>=0; lvl--)
  {
    ...
    Vec6 resOld = calcRes(lvl, T_stereo, scale_current, setting_coarseCutoffTH*levelCutoffRepeat);
    while(resOld[5] > 0.6 && levelCutoffRepeat < 50)
    {
      levelCutoffRepeat*=2;
      resOld = calcRes(lvl, T_stereo, scale_current, setting_coarseCutoffTH*levelCutoffRepeat);
    ...
    for(int iteration=0; iteration < maxIterations[lvl]; iteration++)
    {
      ...
      if(!std::isfinite(inc) || fabs(inc)>scale_current) inc=0.0;

      float scale_new = scale_current + inc;

      Vec6 resNew = calcRes(lvl, T_stereo, scale_new, setting_coarseCutoffTH*levelCutoffRepeat);
...
void ScaleOptimizer::makeCoarseDepthL0(std::vector<FrameHessian*> frameHessians)
{
...
/*
void ScaleOptimizer::calcGSSSE(int lvl, float &H_out, float &b_out, const SE3 &T_stereo, float scale)
{
  H_out=0.0;
  b_out=0.0;
...
Vec6 ScaleOptimizer::calcRes(int lvl, const SE3 &T_stereo, float scale, float cutoffTH)
{
  float E = 0;
  int numTermsInE = 0;
  ...
  for(int i=0;i<nl;i++)
  {
    ...
    if(debugPlot) projImage->setPixel4(Ku, Kv, Vec3b(0,0,255));
