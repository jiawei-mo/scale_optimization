...
void VisualizerSOSVO::publishMinimal(
    const cv::Mat& img,
    const FramePtr& frame,
    const FrameHandlerSOSVO& slam,
...
void VisualizerSOSVO::visualizeMarkers(
    const FramePtr& frame,
...
void VisualizerSOSVO::publishMapRegion(set<FramePtr> frames)
{
...
void VisualizerSOSVO::removeDeletedPts(const Map& map)
{
  if(pub_points_.getNumSubscribers() > 0)
  {
...
void VisualizerSOSVO::displayKeyframeWithMps(const FramePtr& frame, int ts)
{
  // publish keyframe
...
void VisualizerSOSVO::exportToDense(const FramePtr& frame)
{
