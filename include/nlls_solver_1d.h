...
#ifndef LM_SOLVER_1D_H_
#define LM_SOLVER_1D_H_
...
protected:
  double                H_;       //!< Hessian approximation
  double                Jres_;    //!< Jacobian x Residual
  double                x_;       //!< update step
  bool                  have_prior_;
  double prior_;
  double                I_prior_; //!< Prior information matrix (inverse covariance)
  double                chi2_;
  double                rho_;
  Method                method_;
  ...
  virtual double
  computeResiduals      (const double& model,
  ...
  virtual void
  update                (const double& old_model, double& new_model) = 0;

  virtual void
  applyPrior            (const double& current_model) { }
...
public:
  ...
  const double& getInformationMatrix() const;
};

} // end namespace vk

// #include "nlls_solver_1d_impl.hpp"

#endif /* LM_SOLVER_1D_H_ */
